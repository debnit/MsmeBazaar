...
const odishaCoordinates: Record<string, { lat: number; lng: number }> = {
  ...
  Koraput: { lat: 18.812, lng: 82.7115 },
  Angul: { lat: 20.84, lng: 85.1018 },
  ...
  Bolangir: { lat: 20.7117, lng: 83.443 },
  ...
  Ganjam: { lat: 19.3858, lng: 84.88 },
  Gajapati: { lat: 18.7969, lng: 83.9183 },
  Nayagarh: { lat: 20.1333, lng: 85.1 },
};
...
export class MatchmakingEngine {
  ...
  private async getBuyerPreferences(
    buyerId: number,
  ...
  private calculateIndustryMatch(
    ...
    for (const preferredIndustry of preferences.preferredIndustries) {
      const compatibility =
        this.industryCompatibility[preferredIndustry]?.[listingIndustry] || 0;
  ...
  private calculateRiskProfile(
    ...
    const yearsInBusiness =
      ...
      (listing.establishedYear || new Date().getFullYear());
